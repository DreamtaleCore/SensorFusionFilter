...
public class Quaternion implements Serializable {
    ...
    public Quaternion (Point3 axis, double angle) {
        this.set(axis, angle);
    }
     ...
     * @param angle The angle in degrees
     * @return This quaternion for chaining. */
    public Quaternion set (Point3 axis, double angle) {
    ...
    public Quaternion setEulerAnglesRad (double yaw, double pitch, double roll) {
        ...
        x = shp*shy*chr + chp*chy*shr; // x = sin(Y/2)sin(Z/2)cos(X/2)+cos(Y/2)cos(Z/2)sin(X/2)
        y = shp*chy*chr + chp*shy*shr; // y = sin(Y/2)cos(Z/2)cos(X/2)+cos(Y/2)sin(Z/2)sin(X/2)
        z = chp*shy*chr - shp*chy*shr; // z = cos(Y/2)sin(Z/2)cos(X/2)-sin(Y/2)cos(Z/2)sin(X/2)
        w = chp*chy*chr + shp*shy*shr; // w = cos(Y/2)cos(Z/2)cos(X/2)-sin(Y/2)sin(Z/2)sin(X/2)
        return this;
    }
    ...
    // TODO : this would better fit into the Point3 class
    /** Transforms the given vector using this quaternion
     *
     * @param v Vector to transform */
    public Point3 transform (Point3 v) {
        tmp2.set(this);
        tmp2.conjugate();
...
// method
    ...
    public void toEuler(Euler e) {
        e.yaw   = Math.atan2(2*(w*z + x*y), 1 - 2*(y*y + z*z));
        e.pitch = Math.asin(2*(w*y - z*x));
        e.roll  = Math.atan2(2*(w*x + y*z), 1 - 2*(x*x + y*y));
    }
     ...
     * @return This quaternion for chaining. */
    public Quaternion setFromAxis (final Point3 axis, final double degrees) {
     ...
     * @return This quaternion for chaining. */
    public Quaternion setFromAxisRad (final Point3 axis, final double radians) {
    ...
    public Quaternion setFromAxes (boolean normalizeAxes, double xx, double xy, double xz, double yx, double yy, double yz, double zx,
                                   double zy, double zz) {
        if (normalizeAxes) {
            final double lx = 1d / Math.sqrt(xx*xx + xy*xy + xz*xz);
            final double ly = 1d / Math.sqrt(yx*yx + yy*yy + yz*yz);
            final double lz = 1d / Math.sqrt(zx*zx + zy*zy + zz*zz);
            xx *= lx;
            xy *= lx;
            xz *= lx;
            yx *= ly;
     ...
     * @return This quaternion for chaining */
    public Quaternion setFromCross (final Point3 v1, final Point3 v2) {
    ...
    public Quaternion setFromCross (final double x1, final double y1, final double z1, final double x2, final double y2, final double z2) {
        final double dot = MathUtils.clamp((x1*x2 + y1*y2 + z1*z2), -1f, 1f);
        final double angle = (double)Math.acos(dot);
...
// value
    ...
    public double getAxisAngle (Point3 axis) {
     ...
     * @see <a href="http://en.wikipedia.org/wiki/Axis%E2%80%93angle_representation">wikipedia</a>
     * @see <a href="http://www.euclideanspace.com/maths/geometry/rotations/conversions/quaternionToAngle">calculation</a> */
    public double getAxisAngleRad (Point3 axis) {
     ...
     * {@link #getAxisAngleRad(Point3)} to get both the axis and the angle of this rotation. Use
     * {@link #getAngleAroundRad(Point3)} to get the angle around a specific axis.
     * @return the angle in radians of the rotation */
    public double getAngleRad () {
        return (double)(2.0 * Math.acos((this.w > 1) ? (this.w / len()) : this.w));
    }

    /** Get the angle in degrees of the rotation this quaternion represents. Use {@link #getAxisAngle(Point3)} to get both the axis
     * and the angle of this rotation. Use {@link #getAngleAround(Point3)} to get the angle around a specific axis.
     ...
     * @param twist will receive the twist rotation: the rotation around the specified axis
     * @see <a href="http://www.euclideanspace.com/maths/geometry/rotations/for/decomposition">calculation</a> */
    public void getSwingTwist (final Point3 axis, final Quaternion swing, final Quaternion twist) {
    ...
    public double getAngleAroundRad (final double axisX, final double axisY, final double axisZ) {
        ...
        return MathUtils.isZero(l2) ? 0f : (double)(2.0 * Math.acos(MathUtils.clamp(
                (double)((d < 0 ? -this.w : this.w) / Math.sqrt(l2)), -1f, 1f)));
    }

    /** Get the angle in radians of the rotation around the specified axis. The axis must be normalized.
     * @param axis the normalized axis for which to get the angle
     * @return the angle in radians of the rotation around the specified axis */
    public double getAngleAroundRad (final Point3 axis) {
     ...
     * @param axis the normalized axis for which to get the angle
     * @return the angle in degrees of the rotation around the specified axis */
    public double getAngleAround (final Point3 axis) {
